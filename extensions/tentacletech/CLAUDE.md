...
| 4 — Collision | **slices 4A+4B done** | **4A (2026-05-01):** Type-4 environment raycasts (3/tick from base/mid/tip in gravity dir) + normal-only PBD projection scaled by `particle_collision_radius * girth_scale`. Files: `src/collision/environment_probe.{h,cpp}`, `gdscript/debug/gizmo_layers/environment_layer.gd`. New solver API: `set_environment_contacts/clear_environment_contacts/set_collision_radius`. New Tentacle exports: `environment_probe_enabled`, `environment_probe_distance`, `environment_collision_layer_mask`, `particle_collision_radius`. New public `Tentacle.tick(dt)` so headless tests drive the same path as `_physics_process`. Snapshot accessor `get_environment_contacts_snapshot()`. **4B (2026-05-02):** §4.3 unified friction cone projection on top of 4A's normal correction. New header `src/collision/friction_projection.h` with inline `project_friction(particle, n, dn, mu_s, mu_k, out)` — single source of truth so type-1 (slice 4D) reuses the cone math. Solver gained `set_friction(static, kinetic_ratio)` + `get_environment_friction_applied()` per-contact accumulator. Tentacle exports: `base_static_friction` (default 0.4 = §4.4 smooth-vs-skin-dry baseline), `tentacle_lubricity` (0..1; modulates μ_s as `(1-lub)`), `kinetic_friction_ratio` (default 0.8). Snapshot extended with `friction_applied: Vector3` per hit entry. Gizmo layer gets optional `draw_friction_vectors` toggle (lime-green arrows scaled ×200 from per-tick displacement). Plan doc: `docs/Cosmic_Bliss_Update_2026-05-01_phase4_collision.md`. **Tests:** test_collision_type4 7/7 (3 new: friction recorded in snapshot, lubricity=1.0 zeros it, friction-vs-slick lateral drift comparison under tilted gravity). **Deferred (slices 4C+):** §4.3 soft `contact_stiffness` for distance constraints + `in_contact_this_tick` particle flag (slice 4C); type-1 ragdoll reciprocal routing (slice 4D); §4.4 modulator stack (rib / grip / barbed / anisotropy / adhesion); type-1 ragdoll capsules + §4.5 ownership amendment + §4.6 wetness deferred pending Marionette active ragdoll. Pre-existing 2 failures in `test_tentacle_behavior` are unrelated. |
| 5 — Orifice | blocked | |
